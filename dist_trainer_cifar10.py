...
from grace_dl.torch.compressor.lowqsgd import LowQSGDCompressor
from grace_dl.torch.compressor.MGCCompressor import MGCCompressor
from grace_dl.torch.compressor.dgc import DgcCompressor
from grace_dl.torch.compressor.mgc import MGC
...
from grace_dl.torch.memory.mgc import MgcMemory

import models

#print(model_names)

parser = argparse.ArgumentParser(description='Dis-trainer for CIFAR10 in pytorch')
...
parser.add_argument('--log-dir',dest='log_dir',
                    help='TensorboardX log files path',
...
def adjust_learning_rate(optimizer, epoch, size=1):
    """Sets the learning rate to the initial LR decayed by 2 every 30 epochs"""
    lr = args.lr * (0.1 ** (epoch // 80)) * size
...
def train(train_loader, model, criterion, optimizer, epoch, log=None):
    ...
    train_accuracy = top1.avg
    epoch_time = batch_time.sum

    train_loss = metric_average(train_loss, 'avg_train_loss')
    train_accuracy = metric_average(train_accuracy, 'avg_train_accuracy')
    epoch_time = metric_average(epoch_time, 'avg_epoch_time')

    if hvd.rank() == 0:
        log.add_scalar('train/loss', train_loss, epoch)
        log.add_scalar('train/accuracy', train_accuracy, epoch)
        log.add_scalar('train/epoch_time', epoch_time, epoch)
        print('Epoch:{:d}\t'
              'Time:{:.3f}\t'
              'Loss:{:.3f}\t'
              'Acc Top@1 {:.3f}'.format(epoch,epoch_time,train_loss,train_accuracy))
...
def validate(val_loader, model, criterion, epoch, log=None):
    ...
    test_accuracy_1 = top1.avg
    test_accuracy_5 = top5.avg

    test_loss = metric_average(test_loss, 'avg_loss')
    test_accuracy_1 = metric_average(test_accuracy_1, 'avg_accuracy_1')
    test_accuracy_5 = metric_average(test_accuracy_5, 'avg_accuracy_5')

    if hvd.rank() == 0:
        log.add_scalar('test/loss', test_loss, epoch)
        log.add_scalar('test/acc@1', test_accuracy_1, epoch)
        log.add_scalar('test/acc@5', test_accuracy_5, epoch)
        print('\nTest set: Average loss: {:.4f}, Accuracy@1: {:.2f}%, Accuracy@5: {:.2f}%\n'.format(
            test_loss, test_accuracy_1, test_accuracy_5))

   # print(' * Prec@1 {top1.avg:.3f}'
   #        .format(top1=top1))

    return test_accuracy_1, test_accuracy_5
